...
```{r}
md.pattern(df) # understanding the pattern of missing data for smaller data sets
...
# Check which variables are related to missingness - Identifying auxiliary variables
```{r}
...
```
# Linear regression on imputed models using MICE
```{r}
fit <- with(imp_mypred, lm(CBCL ~ crEA))
summary(pool(fit))
```
...
spec_imp = spec_imp %>% # creates data frames within data frames
...
# visualize 
plot_specs(mean_estimate_attach) 
```
